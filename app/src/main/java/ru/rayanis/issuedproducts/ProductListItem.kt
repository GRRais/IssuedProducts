    ...
    Card(
        ...
        Row(Modifier.clickable{navigateToDetails(product)}) {
            Column(
                ...
                product.title?.let { Text(text = it , style = MaterialTheme.typography.h6) }
                product.destination?.let { Text(text = it , style = MaterialTheme.typography.h6) }
                Text(text = product.productCost.toString(), style = MaterialTheme.typography.h6)
                product.description?.let { Text(text = it , style = MaterialTheme.typography.h6) }
            }
        }
    }
}